...
// Función para obtener el id_sensor asociado al correo
...
// Función para obtener los datos de mediciones con el id_sensor específico
async function obtenerDatosMediciones(id_sensor) {
    const response = await fetch(`http://localhost:8080/mediciones/00:1A:2B:3M:4D:5E`);
...
async function agregarChinchetas(id_sensor) {
    ...
    Object.keys(medicionesPorUbicacion).forEach(key => {
        ...
        popupContent += `
        <a href="html/infoExtraPublico.html" 
            style="
                padding: 10px 20px; 
                border-radius: 33px; 
...
// Ahora se usa un correo fijo para obtener los datos
const correo = "visi02@gmail.com";
...
async function obtenerIdSensor(correo) {
    const response = await fetch(`http://localhost:8080/obtenerSensorPorCorreo/visi02@gmail.com`);
    const data = await response.json();
    return data.id_sensor;
}
